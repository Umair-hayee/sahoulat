...
class MessagesController extends Controller
{
    ...
    public function send(Request $request)
    {
        ...
        if (!$error->status) {
            $phonePattern = '/\b(?:\d{10}|\d{3}[-.\s]\d{3}[-.\s]\d{4}|\+\d{1,2}[-.\s]?\d{10})\b/';
            $emailPattern = '/\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b/';
            $toName = User::where('id', $request['id'])->first();
            $adminRole = Role::where('name', 'admin')->first();
            $admin = User::role('admin')->first();
            $data = [
                'to' => $toName ? $toName->name : 'N/A',
                'from' => Auth::user()->name,
                'message' => $request['message'],
            ];
            if (preg_match($phonePattern, $request['message']) || preg_match($emailPattern, $request['message'])) {
                Mail::send('emails.chat-notify', ['emailData' => $data], function($message) use ($admin) {
