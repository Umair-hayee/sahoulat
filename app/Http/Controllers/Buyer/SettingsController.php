...
class SettingsController extends Controller
{
    ...
    public function storeProfileSettings(Request $request)
    {
        ...
        $user = User::where('id', auth()->user()->id)->first();
        $user->update($data);
        return redirect()->route('buyer.settings.index')->with(['success' => 'Profile settings updated successfully!']);
    }
    ...
    public function phoneVerification(Request $request)
    {
        ...
        $user = User::where('id', auth()->user()->id)->first();
        if($request->has('two_factor_authentication')){
            $user->update([
                'two_factor_authentication' => 1
            ]);
            $otp = random_int(100000, 999999);
            $checkOtp = User::where('id', auth()->user()->id)
